...
class TSP:
    ...
    def __init__(self, filename):
        ...
        self._num_pts = int(lines[curr_line])
        curr_line += 1

        print "Number of Cities: ", self._num_pts

        self._X = [ [ Int("x_%s_%s" % (j,i)) for i in range(self._num_pts) ] \
        ...
        for s in range(self._num_pts):
            self._cost_matrix.append( [int(x) for x in lines[curr_line].split()] )
            curr_line += 1

        print "Cost Matrix: "
        pp(self._cost_matrix)
    ...
    def solveTSP(self):
        ...
        for j in range(1, self._num_pts):
            for i in range(1, self._num_pts):
                if i == j:
                    continue
                self._s.add( self._U[i] - self._U[j] + self._num_pts * self._X[i][j] <= self._num_pts - 1 )
        # print self._s
